...
def crop_fits_images(source_folder, destination_folder, x_start, y_start, x_size, y_size):
    ...
    for file_name in os.listdir(source_folder):
        if file_name.lower().endswith(('.fit', '.fits')) and not file_name.startswith("._"):
            file_path = os.path.join(source_folder, file_name)
            logging.info(f"Processing file: {file_name}")

            try:
                with fits.open(file_path) as hdul:
                    # Find first HDU with valid image data
                    image_hdu = None
                    for hdu in hdul:
                    ...
                    if image_hdu is None:
                        logging.warning(f"No valid image data found in {file_name}. Skipping.")
                        continue
                    ...
                    if (y_start + y_size) > image_data.shape[0] or (x_start + x_size) > image_data.shape[1]:
                        logging.warning(f"Skipping {file_name}: Crop dimensions exceed image size.")
                        continue
                    ...
                    # Ensure IMAGEW and IMAGEH reflect cropped size
                    if 'IMAGEW' in new_header:
                        new_header['IMAGEW'] = x_size
                    if 'IMAGEH' in new_header:
                        new_header['IMAGEH'] = y_size
                    ...
                    if 'CRPIX1' in new_header and 'CRPIX2' in new_header:
                        try:
                            ...
                            logging.warning(f"Could not update WCS for {file_name}: {e}")
                    ...
                    new_header.add_history(f"Cropped to {x_size}x{y_size} at ({x_start}, {y_start})")
                    ...
                    new_hdul.writeto(new_file_path, overwrite=True, output_verify='fix')
...
def main():
    ...
    parser.add_argument("--source_folder", type=str, help="Path to source folder containing FITS files.")
    parser.add_argument("--destination_folder", type=str, help="Path to destination folder.")
    parser.add_argument("--x_start", type=int, help="Upper-left corner x-coordinate for cropping.")
    parser.add_argument("--y_start", type=int, help="Upper-left corner y-coordinate for cropping.")
    parser.add_argument("--x_size", type=int, help="Width of crop.")
    parser.add_argument("--y_size", type=int, help="Height of crop.")
    parser.add_argument("--use_gui", action='store_true', help="Use GUI dialogs to select folders even if command-line arguments are provided.")
    ...
    if not args.source_folder or args.use_gui:
        args.source_folder = select_folder("Select Source Folder Containing FITS Files")
